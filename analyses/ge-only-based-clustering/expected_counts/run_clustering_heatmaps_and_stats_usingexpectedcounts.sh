...
-i input/pbta-gene-counts-rsem-expected_count-collapsed.combined.rds  \
-p input/pbta-histologies.tsv \
-c input/initial_hgat_histology.tsv \
-o input/initial_hgat-pbta-gene-counts-rsem-expected_count.rds \
-t input/initial_hgat-pbta-gene-counts-rsem-expected_count.tsv

## Batch correction for samples of interest
Rscript code/02-batch-correct.R \
--metadata input/initial_hgat_histology.tsv \
--mat input/initial_hgat-pbta-gene-counts-rsem-expected_count.rds \
...
--input input/initial_hgat-pbta-gene-counts-rsem-expected_count_corrected.rds \
--gencode_version 27 \
--output input/initial_hgat-pbta-gene-counts-rsem-expected_count_corrected.proteincoding.rds
...
-i input/initial_hgat-pbta-gene-counts-rsem-expected_count_corrected.proteincoding.rds  \
-f input/initial_hgat_histology.tsv -t YES --nclust 5


# VTEST to pick immportant features
python3 code/06-vtest.py \
-i input/initial_hgat-pbta-gene-counts-rsem-expected_count_corrected.proteincoding.tsv \
...
-i input/initial_hgat-pbta-gene-counts-rsem-expected_count_corrected.proteincoding_top10k.tsv \
-c  output/CC_based_heatmap_Distance_spearman_finalLinkage_average_clusterAlg_PAM_expct_counts_edgeR_cluster_and_annotation.tsv \
